...
export function calculateLineItemTax(item: LineItemInput, isInterState: boolean): LineItemTax {
  ...
  const lineTotal = round2(taxableAmount + cgst + sgst + igst + cess);

  return { taxableAmount: round2(taxableAmount), cgst, sgst, igst, cess, lineTotal };
}
...
export function getBranchSupplyStateCode(branch: { gstin?: string | null; stateCode: string }): string {
  return branch.gstin ? stateCodeFromGstin(branch.gstin) : branch.stateCode;
}
...
export function calculateInvoiceTotals(items: LineItemTax[]) {
  ...
  return {
    ...
    roundOff,
    grandTotal: round2(grandTotal + roundOff)
  };
}
